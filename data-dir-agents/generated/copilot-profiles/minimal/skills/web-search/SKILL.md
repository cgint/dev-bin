---
name: web-search
description: Search the web using webs.sh with grounding and url2md.py
---

# Web Search & Grounded Results

This skill provides AI-powered web search capabilities with source-grounded results.

## Tools

**webs.sh** - Web search with grounding; accepts file + prompt, outputs to stdout or file.
...
webs.sh "latest developments in artificial intelligence"

# Search with file input
webs.sh analysis.txt "analyze this information"

# Output to stdout (default)
webs.sh "current model capabilities"
```
...
`webs.sh` uses grounding to tie AI responses directly to real web sources, making outputs more reliable and traceable than standard AI queries.
...
Grounded web search is slow (fetching sources plus model work). Short shell or tool timeouts often fail even when the command would have succeeded.
...
- Performance depends on network access and the configured search backend
