#!/usr/bin/env bash
# Wait for a Herdr agent target and return its lifecycle snapshot plus bounded terminal evidence.
set -uo pipefail
...
usage() {
  cat <<'EOF'
Usage: herdr_await_agent.sh <target> [--timeout-ms <milliseconds>] [--lines <count>]
