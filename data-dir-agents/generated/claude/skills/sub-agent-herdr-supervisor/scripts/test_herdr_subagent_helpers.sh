...
AWAIT="$SCRIPT_DIR/herdr_await_subagent.sh"
FOLLOWUP="$SCRIPT_DIR/herdr_followup_subagent.sh"
...
: >"$TMPDIR_TEST/calls.log"
payload="$(HERDR_AGENT_STATUS=working run_helper "$FOLLOWUP" worker-a 'Pi here: inspect the failure.')"
jq -e '.ok == true and .operation == "followup" and .prompt.sent == false and .prompt.reason == "agent_working" and .agent.status == "working"' <<<"$payload" >/dev/null || fail 'followup did not refuse a busy worker'
if grep -q '^agent prompt ' "$TMPDIR_TEST/calls.log"; then
  fail 'followup sent a prompt to a working worker'
fi

: >"$TMPDIR_TEST/calls.log"
payload="$(HERDR_AGENT_STATUS=blocked run_helper "$FOLLOWUP" worker-a 'Pi here: inspect the failure.')"
jq -e '.ok == true and .prompt.sent == false and .prompt.reason == "agent_blocked" and .agent.status == "blocked"' <<<"$payload" >/dev/null || fail 'followup did not refuse a blocked worker'
if grep -q '^agent prompt ' "$TMPDIR_TEST/calls.log"; then
  fail 'followup sent a prompt to a blocked worker'
fi

: >"$TMPDIR_TEST/calls.log"
payload="$(HERDR_AGENT_STATUS=idle run_helper "$FOLLOWUP" worker-a 'Pi here: inspect the failure.')"
jq -e '.ok == true and .operation == "followup" and .prompt.sent == true and .prompt.exit_code == 0 and .agent.status == "idle" and (.terminal.text | contains("WORK REPORT"))' <<<"$payload" >/dev/null || fail 'followup did not dispatch and collect evidence'
grep -qx 'agent prompt worker-a Pi here: inspect the failure. --wait --timeout 1800000' "$TMPDIR_TEST/calls.log" || fail 'followup used the wrong prompt/wait invocation'

if error_output="$(run_helper "$FOLLOWUP" worker-a '' 2>&1)"; then
  fail 'followup accepted an empty prompt'
fi
...
printf 'PASS: Herdr subagent helper contract\n'
