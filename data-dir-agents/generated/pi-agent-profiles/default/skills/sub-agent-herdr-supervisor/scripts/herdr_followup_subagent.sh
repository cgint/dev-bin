#!/usr/bin/env bash
# Send one follow-up to an idle worker and return lifecycle plus terminal evidence.
set -uo pipefail
...
usage() {
  cat <<'EOF'
Usage: herdr_followup_subagent.sh <target> <message> [--timeout-ms <milliseconds>] [--lines <count>]

Preflights the worker. If it is working, sends nothing and returns evidence with
prompt.sent=false. Otherwise sends one message through `herdr agent prompt --wait`,
then returns one JSON document with prompt, lifecycle, and terminal evidence.
...
if [[ "$(jq -r '.exit_code' <<<"$preflight_response")" != 0 ]]; then
  ...
  herdr_helper_emit followup 'null' '{"sent": false, "reason": "preflight_unavailable"}'
  exit 0
fi
...
if [[ "$HERDR_HELPER_AGENT_STATUS" == working || "$HERDR_HELPER_AGENT_STATUS" == blocked ]]; then
  ...
  herdr_helper_emit followup 'null' "$prompt_payload"
  exit 0
fi
...
herdr_helper_collect_agent
herdr_helper_collect_terminal
herdr_helper_emit followup "$prompt_capture" "$prompt_payload"
