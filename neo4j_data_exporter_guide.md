# Neo4j Data Export Guide (UV Standalone Script)

This guide provides a high-performance Python script for exporting data from live Neo4j database instances into structured JSONL and CSV files. The script is designed as a self-contained UV standalone script, managing its dependencies automatically.

## Prerequisites

Before running the script, ensure you have `uv` installed. If not, you can install it via:
...
## The High-Performance Neo4j Export Script

This script is optimized for handling large datasets by employing streaming, ID-based pagination, and concurrent processing. It saves nodes and relationships into separate JSONL (JSON Lines) and CSV files, along with database statistics and schema information.

Save the following content as a Python file (e.g., `export_neo4j.py`):
...
def main():
    ...
    # Configuration for your Neo4j instances
    # IMPORTANT: Replace 'your_password' with your actual Neo4j passwords
    # Adjust URI/ports if your instances are not on localhost or use different ports
    configs = [
        {
            'name': 'Knowledge Graph',
            'uri': 'bolt://localhost:7687',  # Example: Adjust if needed
            'username': 'neo4j',
            'password': 'your_knowledge_graph_password', # <-- UPDATE THIS
            'database': 'neo4j'
        },
        {
            'name': 'MCP Server',
            'uri': 'bolt://localhost:7688',  # Example: Adjust if needed
            'username': 'neo4j',
            'password': 'your_mcp_server_password', # <-- UPDATE THIS
            'database': 'neo4j'
        }
    ]
    ...
    for config in configs:
        ...
        exporter = None
        try:
            # Create a unique export directory per database
            export_base_name = config['name'].lower().replace(' ', '_')
            ...
            exporter.export_dir = f"{export_base_name}_export_{current_time}"
...
1.  **Save the Script**: Save the Python code above into a file named `export_neo4j.py` (or any `.py` extension).
2.  **Make Executable (Optional, but recommended)**:
    ```bash
    chmod +x export_neo4j.py
    ```
3.  **Update Configuration**: Open `export_neo4j.py` and update the `configs` list within the `main()` function with your actual Neo4j instance URIs, usernames, and **passwords**.
4.  **Run the Script**: Execute the script using `uv run`. `uv` will automatically detect the inline dependencies, create a temporary virtual environment, install `neo4j` and `pandas`, and then run your script.

    ```bash
    uv run python export_neo4j.py
    ```
    (or `./export_neo4j.py` if you made it executable)

## Output Files

The script will create timestamped directories (e.g., `knowledge_graph_export_YYYYMMDD_HHMMSS/`, `mcp_server_export_YYYYMMDD_HHMMSS/`) for each database. Each directory will contain:

*   `nodes.jsonl.gz` (or `.jsonl`): All nodes, one JSON object per line, compressed if `compress_output` is True.
*   `nodes.csv`: All nodes in CSV format.
*   `relationships.jsonl.gz` (or `.jsonl`): All relationships, one JSON object per line, compressed.
*   `relationships.csv`: All relationships in CSV format.
*   `database_stats.json`: Overview of node labels, relationship types, and their counts.
*   `schema.json`: Detailed schema information (node and relationship properties).
*   `export_summary.json`: A summary of the export process, including duration and counts.
*   `export.log`: Detailed log of the export process.

## Performance Considerations for Large Datasets

The script includes optimizations for large datasets:

*   **Streaming Output**: Data is written to files in batches, minimizing in-memory requirements.
*   **ID-Based Pagination**: Queries use Neo4j's internal IDs for efficient pagination, avoiding the performance issues of `SKIP`/`LIMIT` on large offsets.
*   **Parallel Processing**: Node export can be parallelized by label for very large graphs.
*   **Retry Logic**: Basic retry mechanism for database queries to handle transient network issues.

For extremely large databases (10M+ nodes/relationships), consider these additional database-level optimizations:

*   **Create Indexes**: Ensure `id(n)` and `id(r)` indexes exist in your Neo4j database for faster ID-based queries:
    ```cypher
    CREATE INDEX node_id_index IF NOT EXISTS FOR (n) ON (id(n));
    CREATE INDEX rel_id_index IF NOT EXISTS FOR ()-[r]-() ON (id(r));
    ```
*   **Increase Neo4j Memory**: Adjust Neo4j's heap and page cache memory settings:
    ```bash
    ...
    ```
*   **APOC Export**: For massive exports, Neo4j's APOC plugin offers highly optimized export procedures directly from the database server, which might be even more efficient than external scripts if APOC is installed and configured on your Neo4j instances.
    ```cypher
